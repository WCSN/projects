...
function cleaning()
{
    ...
    if [ -d ./work ]; then
		rm -Rf ./work
		rm -Rf "$CDIR/.settings/CMakeLists.txt"
	fi
...
function makeprj()
{
    ...
    if ! [[ -f "$CDIR/work/$DIRBLD/Makefile" ]]; then
        cat "$CDIR/.settings/CMakeLists.ver" > "$CDIR/.settings/CMakeLists.txt"        
        echo "project($PROJECT_NAME)" >> "$CDIR/.settings/CMakeLists.txt"
        cat "$CDIR/.settings/CMakeLists.inc" >> "$CDIR/.settings/CMakeLists.txt"
        cmake -D CMAKE_BUILD_TYPE=$BTYPE -D PROJECT_ROOT_PATH="`pwd`/../.." "$CDIR/.settings/"
    fi
